...
from .const import WakeWordModel
async def async_setup_entry(hass, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    coordinator = hass.data[DOMAIN][entry.entry_id]
    async_add_entities([WakeWordSelect(d) for d in coordinator.devices.values()], True)
class WakeWordSelect(SelectEntity):
    _attr_has_entity_name = True
    _attr_entity_category = 'config'
    _attr_options = [m.value for m in WakeWordModel]
    def __init__(self, device): self._device=device; self._attr_unique_id=f"{device.device_id}_wake_word_model"; self._attr_name='Wake Word Modell'
    @property
    def current_option(self): return self._device.config.get('wake_word_model')
    @property
    def available(self): return self._device.is_connected
    @property
    def device_info(self): return device_info(self._device)
    async def async_select_option(self, option: str): self._device.update_config({'wake_word_model': option})
